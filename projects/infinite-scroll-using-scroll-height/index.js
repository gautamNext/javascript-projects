...
function handleScroll() {
  const bottonHeightLeftToScroll =
    this.scrollHeight - (this.clientHeight + this.scrollTop);
  if (bottonHeightLeftToScroll === 0) {
    fetchTestimonials();
  }
}
...
function fetchNextTextimonialSet(limit = 10) {
  let hasNext = true;
...
const fetchTestimonials = fetchNextTextimonialSet(10);
